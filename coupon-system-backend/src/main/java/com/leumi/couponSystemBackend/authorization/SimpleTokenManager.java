package com.leumi.couponSystemBackend.authorization;
...
public class SimpleTokenManager {
	...
	private static final long EXPIRATION_TIME_PERIOD_IN_MILLIS = 1000*60 * 30; //30 min
	private static final long EXPIRATION_THREAD_PERIOD_IN_MILLIS = 1000*5; //5 seconds
	private static long lastToken = 5678;
	private Map<String, Token> tokens = new HashMap<String, SimpleTokenManager.Token>();
	...
	synchronized public void removeToken(String token) {
		tokens.remove(token);
	}
	...
	synchronized private void removeExpiredSessions() {
		System.out.println("---------------------------------------------------------");
		Map<String, Token> remainedTokens = new HashMap<String, SimpleTokenManager.Token>();
